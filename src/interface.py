...
def start_gui():
    ...
    ttk.Label(window, text="Trials per tasks", font=FONT).grid(row=0, column=0, padx=10, pady=(30, 10))
    ...
    ttk.Label(window, text="Trial max time (sec) ", font=FONT).grid(row=1, padx=10, pady=10)
    ...
    ttk.Label(window, text="Side:", font=FONT).grid(row=2, padx=10, pady=10)
...
def on_click(window, repeat, time, side):
    global config
    window.destroy()
    config = [int(repeat.get()), str(time.get()).replace(":", "."), side.get()]
...
def cal_window_geometry(window):
    ...
    return f'{window_width}x{window_high}+{int((screen_width - window_width) / 2)}+{int((screen_height - window_high) / 2)}'
